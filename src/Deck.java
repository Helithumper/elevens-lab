...
public class Deck {
	...
	public Deck(String[] ranks, String[] suits, int[] values) {
		...
		int smallest = ranks.length;
		smallest = (suits.length<smallest) ? suits.length : ranks.length;
		smallest = (smallest>values.length) ? values.length : smallest;
		int count = 0;
		for (int i = 0; i < smallest; i++) {
			cards.add(new Card(ranks[i%ranks.length], suits[i%suits.length], values[i%values.length]));
		}
		shuffle();
	}
	...
	public Card deal() {
		Card result;
		result = (cards.size() > 0) ? cards.get(cards.size() - 1) : null;
		return result;
	}
